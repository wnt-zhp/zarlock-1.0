...
public class MealProductTableModel 
  ...
  private MealProductTableModel THIS = this;
  
  
  public MealProductTableModel() {
    super();
  ...
  public MealProductTableModel(ProductBaseImmutable base, Component owner) {
    super();
    this.setBase(base);
    this.setOwner(owner);
    fireTableDataChanged();
  ...
  public Object getValueAt(int row, int column) {
    ...
    case CURRENTQUANTITYCOLUMNNUMBER:
     ...
     case DODAJ_COLUMN_NUMBER:
   	  try {
				return productVect.get(row).getId();
			} catch (ZZIllegalEx e) {
				return null;
			}
    }
  ...
  public final static int DESCRIPTIONCOLUMNNUMBER = 4;
  public final static int EXPIRYDATECOLUMNNUMBER = 5;
  public final static int DODAJ_COLUMN_NUMBER = 6;
  ...
  static{
	  ...
	  allColNames.put(new Integer(DODAJ_COLUMN_NUMBER),"Dodaj");  
  };
  ...
  private ProductBaseImmutable.FilteredListener filteredListener 
    = new ProductBaseImmutable.FilteredListener(){
      public void actionPerformed(ProductBaseImmutable filtered, Vector<Query> query) {
      	if(!(filtered.getNumberOfElems()==getBaseFiltered().getNumberOfElems()))
